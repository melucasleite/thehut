...
scheduler = BackgroundScheduler(
    jobstores={'default': SQLAlchemyJobStore(url=os.environ.get(
        "SQLALCHEMY_DATABASE_URI", 'sqlite:///jobs.sqlite'
    ))})
scheduler.start()
scheduler.add_job(generate_lecture_history, id="generate_lecture_history", replace_existing=True, trigger="interval",
                  minutes=5)
scheduler.add_job(generate_payments, id="generate_payments", replace_existing=True, trigger="interval", days=1)
